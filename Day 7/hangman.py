...
from utility_files import word_list

chosen_word = random.choice(word_list) # Choosing a word randomly
...
while ('_' in display != 0 and lives > 0): #looping while there are _s left and lives > 0
    ...
    if guess in chosen_word:
        for index, letter in enumerate(chosen_word):# replacing _ with guessed letter in display
            if letter == guess:
